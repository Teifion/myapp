defmodule MyApp.Account.UserLib do
  @moduledoc """
  Library of user related functions.
  """
  use MyAppMacros, :library
  alias MyApp.Account.{User, UserQueries}

  @doc false
  @spec user_topic(MyApp.user_id() | User.t()) :: String.t()
  def user_topic(%User{id: user_id}), do: "MyApp.Account.User:#{user_id}"
  def user_topic(user_id), do: "MyApp.Account.User:#{user_id}"
  ...
  """
  @spec list_users(MyApp.query_args()) :: [User.t()]
  ...
  """
  @spec get_user!(MyApp.user_id()) :: User.t()
  @spec get_user!(MyApp.user_id(), MyApp.query_args()) :: User.t()
  ...
  """
  @spec get_user(MyApp.user_id(), MyApp.query_args()) :: User.t() | nil
  def get_user(user_id, query_args \\ []) do
  ...
  ## Examples
      ...
      nil
  """
  @spec get_user_by_id(MyApp.user_id()) :: User.t() | nil
  ...
  @spec do_get_user_by_id(MyApp.user_id()) :: User.t() | nil
  ...
  ## Examples
      ...
      iex> get_user_by_email("noodle@myapp")
      %User{}
  ...
  defp maybe_decache_user({:ok, user}) do
    MyApp.invalidate_cache(:user_by_user_id_cache, user.id)
    {:ok, user}
  end
  ...
  def register_failed_login(user_id, ip, reason) do
    ...
    :telemetry.execute(
      [:myapp, :user, :failed_login],
      %{reason: reason},
      %{user_id: user_id, ip: ip}
    )

    # TODO: Call out to the audit log system
    # MyApp.create_audit_log(user_id, ip, "failed-login", %{reason: reason})
  ...
  defp allow_user_login_attempt?(userid) do
    ...
    # max_allowed_user = MyApp.Settings.get_server_setting_value("login.user_rate_limit")
  ...
  ## Examples
    ...
    false
  """
  @spec allow?(MyApp.user_id() | User.t(), [String.t()] | String.t()) :: boolean
