defmodule MyApp.Account.User do
  @moduledoc """
  # User
  The database representation of a person using/playing your game; not to be confused with `MyApp.Connections.Client` which represents their logged in and online state.

  ### Attributes

  * `:name` - The name of the user
  * `:email` - The email of the user
  * `:password` - The encrypted password of the user; if you ever want to compare the password please make user of `valid_password?/2` (which can be called from `MyApp.Account`).
  ...
  * `:shadow_banned?` - Boolean flag for spambots and trolls, when set to true the user should be allowed to see things as if fully logged in and be sent "success" type messages. In reality they are not able to effect change.
  ...
  defp validate_password(changeset) do
    min_length = Application.get_env(:myapp, :default_min_user_password_length, 6)
  ...
  def calculate_user_permissions(changeset) do
    if Application.get_env(:myapp, :fn_calculate_user_permissions) do
      f = Application.get_env(:myapp, :fn_calculate_user_permissions)
      f.(changeset)
    else
